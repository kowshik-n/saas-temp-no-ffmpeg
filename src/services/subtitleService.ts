...
function timeToMilliseconds(timeString: string): number {
  ...
  return (
    hours * 3600000 + minutes * 60000 + seconds * 1000 + parseInt(milliseconds)
  );
}
...
export async function importSRT(
  ...
  // This function should be imported from projectService.ts
  throw new Error("Use saveSubtitles from projectService instead");
}
